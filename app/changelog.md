...
- we are testing stream consistency with respect to cache isolation
  and concurrency
- Following are the tests we want to do:
...
Test B
Final persisted response correct?
PASS / FAIL
...
Test D
Logs show overlapping streams?
PASS / FAIL
```

- so we have added a mock provider and a test class,
- now what we need to do is to route conversation service via that route
- and make some concurrent requests to our backend and see the cache status
- and also the repository status and check if they succeeded or failed
